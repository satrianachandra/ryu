...
class RestQoSAPI(app_manager.RyuApp):
    ...
    def conf_switch_set_handler(self, ev):
        ...
        if ev.key == cs_key.UNIX_SOCKET:
            QoSController.set_unix_socket(ev.dpid, ev.value)
        else:
...
class QoSController(ControllerBase):
    ...
    @staticmethod
    def set_unix_socket(dpid, value):
        ofs = QoSController._OFS_LIST.get(dpid, None)
        if ofs is not None:
            ofs.set_unix_socket(dpid, value)
...
class QoS(object):
    ...
    def set_unix_socket(self, dpid, unix_socket):
        old_address = self.unix_socket
        if old_address == unix_socket:
            return
